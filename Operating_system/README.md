...
# 死锁
- 死锁发生的必要条件：1. 资源互斥 2. 占用且等待 3. 非抢占 4. 循环等待
- 死锁预防：破坏任意一个必要条件。
   1. 破坏资源互斥：使用可共享的资源例如只读文件。
   2. 破坏持有且等待：进程执行前预先申请所有需要的资源；或者进程在申请新的资源的时候必须释放掉当前拥有的所有资源。利用率低，可能发生饥饿。
   3. 破坏非抢占：进程在申请资源时可以抢占已经被分配给别的进程的资源。
   4. 破坏循环等待：对多有资源类型进行排序，每个进程都必须按照固定的顺序申请资源。
- 死锁避免
   1. 安全序列：按照安全序列为进程分配资源可以保证不发生死锁，即对于任意进程Pi所需要的最大资源数<=当前等于资源数+所有在序列中排在Pi前的进程所占用的所有资源。
   2. 安全状态：存在一个安全序列，则表示系统处于安全状态。
   3. 银行家算法：在每次分配之前，先判断此次能否让系统继续保持安全状态，如果可以则分配，不可以则拒绝分配。